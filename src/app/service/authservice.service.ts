...
export class oAuthService {
  integration: any;
  constructor(
    private AccountAuth: AuthService,
  ...
  public async login(): Promise<void> {
    ...
    try {
      ...
      // const data = {
      //   token_type: Loggedin_user['token_type'],
      ...
      const subsiddata = JSON.parse(
        localStorage.getItem('gigaaa-subscription')
      );
      const subsid = await this.gigaaaApiService.getsubsid(
        subsiddata.access_token
      );
      localStorage.setItem(
        'gigaaa-user',
        JSON.stringify(this.AccountAuth.getLoggedUser())
      );
      console.log(subsid);
      ...
      this.gigaaaapi.getallintegration(accesstoken, uuid).subscribe((data) => {
        ...
        this.integration.forEach((element) => {
          if (element.last_used === true) {
            ...
            this.gigaaaapi
              .getloggedinagentuuid(accesstoken, uuid, element.uuid)
              .subscribe((data) => {
                console.log(data);
